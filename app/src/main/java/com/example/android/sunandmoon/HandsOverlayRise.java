...
public class HandsOverlayRise implements DialOverlayRise {
    ...
    public HandsOverlayRise(Context context, boolean useLargeFace) {
    ...
    public HandsOverlayRise(Drawable hourHand, Drawable minuteHand) {
        mUseLargeFace = false;
    ...
    public HandsOverlayRise(Context context, int hourHandRes, int minuteHandRes) {
        final Resources r = context.getResources();
    ...
    public static float getHourHandAngle(int h, int m) {
        return ClockActivityRise.is24 ? ((h/48.0f) * 360) % 360 + (m / 60.0f) * 360 / 24.0f : ((h/ 24.0f) * 360) % 360 + (m / 60.0f) * 360 / 12.0f;
    }
    ...
    public void onDraw(Canvas canvas, int cX, int cY, int w, int h, int hours, int minutes, int seconds,
        ...
        canvas.save();
        if (!ClockActivityRise.hourOnTop)
